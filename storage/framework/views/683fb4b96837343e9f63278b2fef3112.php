        ...
        <aside class="bg-slate-900 text-slate-200 h-full flex flex-col justify-between transition-all duration-300 shadow-xl z-20" :class="sidebarOpen ? 'w-64' : 'w-20'">
            <div>
                ...
                <nav class="mt-6 px-3 space-y-1">
                    <a href="/dashboard" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all <?php echo e(request()->is('dashboard') ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 text-slate-400'); ?>">
                        <span>📊</span>
                        <span x-show="sidebarOpen" class="text-sm font-medium">Dashboard</span>
                    </a>

                    <?php if(auth()->check() && auth()->user()->role === 'admin'): ?>
                        <a href="/admin/calendar" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all <?php echo e(request()->is('admin/calendar') ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 text-slate-400'); ?>">
                            <span>📅</span>
                    ...
                    <?php if(auth()->check() && auth()->user()->role !== 'admin'): ?>
                        <a href="/appointments/book" class="flex items-center gap-4 px-4 py-3 rounded-xl transition-all <?php echo e(request()->is('appointments/book') ? 'bg-blue-600 text-white' : 'hover:bg-slate-800 text-slate-400'); ?>">
                            <span>💼</span>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
            <header class="h-16 bg-white border-b border-slate-200 flex items-center px-6 shadow-sm">
                <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    <?php echo e(auth()->user()->role ?? 'Guest'); ?> Portal
                </span>
            </header>
...
</html><?php /**PATH C:\xampp\htdocs\appointment-system\resources\views/components/layout.blade.php ENDPATH**/ ?>