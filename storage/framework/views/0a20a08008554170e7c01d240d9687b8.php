...
<?php $component->withAttributes([]); ?>
    <div class="max-w-2xl mx-auto bg-[#1a1a1e] rounded-2xl shadow-xl border border-slate-800/80 p-6 sm:p-8 mt-4">
        ...
        <form action="<?php echo e(route('appointments.store')); ?>" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <select name="service_ids[]" multiple required size="5" class="w-full border border-slate-800 rounded-xl px-4 py-3 bg-[#121214] text-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm transition-all">
                    <?php $__currentLoopData = $services; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $s): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <option value="<?php echo e($s->id); ?>"><?php echo e($s->name); ?> — ($<?php echo e(number_format($s->price, 2)); ?>)</option>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </select>
            ...
            <div x-data='{
                    schedules: <?php echo json_encode($schedulesByDate, 15, 512) ?>,
                    selectedDate: "<?php echo e(old('schedule_date', now()->format('Y-m-d'))); ?>",
                    selectedScheduleId: "",
                    customStartTime: "<?php echo e(old('schedule_start_time', '')); ?>",
                    customEndTime: "<?php echo e(old('schedule_end_time', '')); ?>",
                    get dates() {
            ...
            <?php if(isset($userAppointments) && $userAppointments->isNotEmpty()): ?>
                <div class="rounded-2xl border border-slate-800 bg-[#121214] p-4">
                    <h2 class="text-sm font-semibold text-white mb-3">Your existing appointments</h2>
                    <div class="space-y-3 text-slate-300 text-sm">
                        <?php $__currentLoopData = $userAppointments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $appt): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="rounded-xl border border-slate-700 bg-slate-900/60 p-3">
                                <div class="font-semibold text-white"><?php echo e($appt->services->isNotEmpty() ? $appt->services->pluck('name')->join(', ') : $appt->service->name); ?></div>
                                <div class="text-slate-400 text-xs"><?php echo e($appt->schedule->available_date); ?> @ <?php echo e($appt->schedule->start_time); ?> - <?php echo e($appt->schedule->end_time); ?></div>
                                <div class="text-slate-400 text-xs">Status: <?php echo e(ucfirst($appt->status)); ?></div>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </div>
                </div>
...
<?php endif; ?><?php /**PATH C:\xampp\htdocs\appointment-system\resources\views/customer/book.blade.php ENDPATH**/ ?>